...
export default definePlugin({
    ...
    start() {
        ...
        showToast("Quest Auto-Complete loaded!", Toasts.Type.SUCCESS);
    },

    stop() {
        this.removeButton();
        showToast("Quest Auto-Complete unloaded!", Toasts.Type.INFO);
    },
    ...
    async runQuestAutomation(button) {
        try {
            ...
            if (!ApplicationStreamingStore || !RunningGameStore || !QuestsStore || !ChannelStore || !GuildChannelStore || !FluxDispatcher || !api) {
                showToast("Failed to find required Discord stores. Please refresh and try again.", Toasts.Type.FAILURE);
            ...
            if (activeQuests.length === 0) {
                showToast("No enrolled/uncompleted quests found!", Toasts.Type.MESSAGE);
                this.resetButton(button);
                return;
            }
            ...
            showToast(`Starting ${activeQuests.length} quest(s)...`, Toasts.Type.MESSAGE);
            ...
            Promise.all(activeQuests.map(quest => completeQuest(quest)))
                .then(() => {
                    ...
                    showToast("All quests completed!", Toasts.Type.SUCCESS);
                ...
                .catch(error => {
                    ...
                    console.error(error);
                    showToast("Quest automation failed! Check console.", Toasts.Type.FAILURE);
                    this.resetButton(button);
                });

        } catch (error) {
            console.error("Failed to run quest automation:", error);
            showToast("Failed to run automation. Check console for details.", Toasts.Type.FAILURE);
            this.resetButton(button);
        }
    }
});
